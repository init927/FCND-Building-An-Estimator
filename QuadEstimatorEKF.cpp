...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  // creating rotation matrix
    ...
    Mat3x3F rotm = Mat3x3F::Zeros();
    float phi_angle= rollEst;
    float theta_angle = pitchEst;
    rotm(0,0) = 1;
    rotm(0,1) = sin(phi_angle) * tan(theta_angle);
    rotm(0,2) = cos(phi_angle) * tan(theta_angle);
    rotm(1,1) = cos(phi_angle);
    rotm(1,2) = -sin(phi_angle);
    rotm(2,1) = sin(phi_angle) / cos(theta_angle);
    rotm(2,2) = cos(phi_angle) / cos(theta_angle);

    V3F angle_dot = rotm*gyro;
    
// predicting roll and pitch of the angle
    
    float predictedRoll = rollEst+dtIMU*angle_dot.x; // x-axis
    
    float predictedPitch = pitchEst+dtIMU*angle_dot.y; // y-axis
    
    ekfState(6) = ekfState(6) + dtIMU * angle_dot.z; // z-axis
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    // based on accelartion measurements
    ...
    predictedState(5) = curState(5) + dt * acceleration.z - dt * CONST_GRAVITY;
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    //GetRbgPrime is used for getting the first order derivates of rotation matrix `Rbg`, which can be used for constructing the final jacobian matrix, the key for updating sigma.
    float cos_theta,sin_theta,cos_phi,sin_phi,sin_psi,cos_psi;
    cos_theta = cos(pitch);
    sin_theta = sin(pitch);
    cos_phi = cos(roll);
    sin_phi = sin(roll);
    sin_psi = sin(yaw);
    cos_psi = cos(yaw);
    //https://www.overleaf.com/read/vymfngphcccj#/54894644/ 7.2 rbg
...
void QuadEstimatorEKF::UpdateFromGPS(V3F pos, V3F vel)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    for(int i=0;i<6;i++)
    {
        hPrime(i,i) = 1; // diagonal elements equal to 1
    }
  /////////////////////////////// END STUDENT CODE ////////////////////////////
